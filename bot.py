...
# Настройка DeepSeek клиента (это не Pyrogram, можно оставить здесь)
deepseek_client = openai.OpenAI(
    api_key=DEEPSEEK_API_KEY,
    base_url="https://api.deepseek.com/v1"
)

# Хранилище сообщений по чатам (будет общим для всех потоков)
message_store = defaultdict(list)
MAX_MESSAGES_PER_CHAT = 5000

# ============================================
# ВСЯ ЛОГИКА БОТА В ОДНОЙ ФУНКЦИИ
# ============================================
def run_bot():
    """Функция запуска бота. Импорт Pyrogram внутри!"""

    # Импортируем Pyrogram ТОЛЬКО здесь, когда цикл событий уже создан
...
if __name__ == "__main__":
    ...
    # 1. СОЗДАЕМ ЦИКЛ СОБЫТИЙ
    loop = asyncio.new_event_loop()
    asyncio.set_event_loop(loop)

    # 2. Функция для запуска бота в потоке
    def bot_thread_target():
        asyncio.set_event_loop(loop)
        run_bot()  # В этой функции уже есть импорт Pyrogram

    # 3. Запускаем бота в фоновом потоке
    bot_thread = threading.Thread(target=bot_thread_target, daemon=True)
    bot_thread.start()

    # 4. HTTP-сервер для Health Checks Render
